...
`3.3\cdot10^{18}` in a Training Cost column. That is correct, because lower is better.
...
in the current source, and our canonical end-to-end test case.
...
codebase. Read it before a refactor, and rebuild after structural changes:
...
agents colliding, and they also stop agents seeing each other's code. That is how this
...
`not_attempted` has no §7 label; it renders as **not checked**, which matches the §5.5
section name. It is a normal outcome for a paper with no URLs or no bibliography, and it
must not read as a failure.

## Environment

- Python 3.14.3, Node 24.14.1, git. Docker is not installed, so there is no Postgres yet.
