  ...
  return (
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col two:flex-row">
        {/* Stacked above the document below 760px, so it is capped there — a run
            with twelve checks must not squeeze the paper off the screen. */}
        <aside className="flex max-h-[45dvh] shrink-0 overflow-hidden two:max-h-none two:h-full two:w-[var(--run-rail-w)]">
          <RunRail
            report={report}
            rows={rows}
            ...
            onSelect={select}
            // The pane's resting content is the not-checked list, so showing it
            // means clearing the selection — and opening the sheet below 1100px.
            onShowNotChecked={() => {
              setSelectedChecker(null);
              setSheetOpen(true);
            }}
        ...
        {/* Content region — document / gutter / verdict at 55/45 with a fixed
            48px gutter track between them. */}
